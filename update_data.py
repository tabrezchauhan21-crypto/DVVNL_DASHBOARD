"""
update_data.py
Fetches formdata.json from SharePoint and regenerates data_v4.js
Run by GitHub Actions every hour OR run manually anytime.
"""
...
# ── SharePoint URL (from GitHub Secret) ──────────────────────────────────────
SP_URL = os.environ.get('SP_URL', '')

if not SP_URL:
    print("SP_URL not set — using local formdata.json if available")
    try:
        with open('formdata.json') as f:
            raw = json.load(f)
        print(f"Loaded local formdata.json: {len(raw)} records")
    except:
        print("No local formdata.json found. Exiting.")
        exit(0)
else:
    ...
    r.raise_for_status()
    raw = r.json()
    print(f"Fetched {len(raw)} records from SharePoint")
...
for mon, yr in all_months:
    ...
    vd = set(r['dist_div'].upper().strip() for r in recs
             if r['verified_by']=='EX EN DISTRIBUTION' and r['dist_div'])
    vt = set(r['test_div'].upper().strip() for r in recs
             if r['verified_by']=='EX EN TEST' and r['test_div'])
    vc = set(r['circle'].upper().strip() for r in recs
             if r['verified_by']=='CIRCLE OFFICE - SE' and r['circle'])
...
with open('data_v4.js','w',encoding='utf-8') as f:
    f.write(f'// Auto-generated: {ts} | Records: {len(records)} | Months: {len(MONTHLY_STATUS)}\n')
    f.write(f'const MASTER_ALL={json.dumps(MASTER_ALL, separators=(",",":"))    };\n')
    f.write(f'const MONTHLY_STATUS={json.dumps(MONTHLY_STATUS, separators=(",",":"))  };\n')
    f.write(f'const FORM_RECORDS={json.dumps(mini, separators=(",",":"))};\n')

import os
print(f"\ndata_v4.js written: {os.path.getsize('data_v4.js')//1024} KB")
print("Done!")
